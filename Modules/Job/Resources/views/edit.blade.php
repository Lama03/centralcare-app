                ...
                <div class="card">
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.jobs.update', ['job' => $form->id]) }}">
                                @method('PUT')
                                @csrf
                                ...
                                <div class="form-row " style="margin-bottom:  10px">
                                            ...
                                            @foreach(config('translatable.locales') as $locale)
                                                <div class="tab-pane fade {{ $loop->first ? "active show" : '' }}" id="{{ $locale }}" role="tabpanel">
                                                    <div class="pt-4">
                                                        ...
                                                        <!-- @include('admin.layout.field', field('textarea', $locale.'[content]', 'Content', 6, $form->translate($locale), ['key' => 'content'])) -->
                                                        ...
                                                        <div class="form-group bmd-form-group col-md-6 @error("$locale.description") has-danger @enderror">
                                                            ...
                                                            <textarea id="description_name" name="{{ $locale }}[description]" class="form-control" rows="10">{{ $form->translate($locale)->description }}</textarea>

                                                            @error("$locale.description")
                                                                <span class="text-danger">{{ $message }}</span>
                                                            @enderror
                                                        </div>
                                                        <!-- @include('admin.layout.field', field('textarea', $locale.'[requirements]', 'Requirements', 6, $form->translate($locale), ['key' => 'requirements'])) -->
                                                        <div class="form-group bmd-form-group col-md-6 @error("$locale.requirements") has-danger @enderror">
                                                            <label for="requirements_id"> Requirements {{ $locale }} <span class="text-danger">*</span></label>
                                                            <textarea id="requirements_name" name="{{ $locale }}[requirements]" class="form-control" rows="10">{{ $form->translate($locale)->requirements }}</textarea>
                                    ...
                                    <!-- @include('admin.layout.field', field('select', 'department_id', 'Department', 6, $form, $departments)) -->
                                    <div class="form-group col-md-6 mt-3 @error('department_id') has-danger @enderror">
                                            ...
                                            <select class="custom-select form-control city-type" id="department_id" name="department_id">
                                                ...
                                                @if(!empty($departments))
                                                    ...
                                                    <option value="{{$depart->id}}" {{ $form->department_id == $depart->id ? 'selected' : '' }}>{{$depart->name}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
