                ...
                <div class="card">
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.devices.store') }}">
                                @method('POST')
                                @csrf
                                                ...
                                                <div class="tab-pane fade {{ $loop->first ? "active show" : '' }}" id="{{ $locale }}" role="tabpanel">
                                                    <div class="pt-4">
                                                        <div class="form-group col-md-12">
                                                            <label for="select_page_id"> Name {{ $locale }}</label>
                                                        ...
                                                        <div class="form-group col-md-12">
                                                            <label for="select_page_id"> Features {{ $locale }}</label>
                                                            <textarea id="input_page_name" name="{{ $locale }}[features]" class="form-control" rows="15">{{ old("$locale.features") }}</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
