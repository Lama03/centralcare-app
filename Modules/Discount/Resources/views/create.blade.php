                ...
                <div class="card">
                    <div class="card-body">
                        <div class="basic-form">
                            <form class="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.discounts.store') }}">
                                @method('POST')
                                @csrf
                                ...
                                <div class="form-row " style="margin-bottom:  10px">
                                            ...
                                            @foreach(config('translatable.locales') as $locale)
                                                <div class="tab-pane fade {{ $loop->first ? "active show" : '' }}" id="{{ $locale }}" role="tabpanel">
                                                    <div class="pt-4">
                                                        ...
                                                        <!-- @include('admin.layout.field', field('text', $locale.'[description]', 'Slug', 6, [], ['key' => 'description'])) -->
                                                        ...
                                                        <!-- @include('admin.layout.field', field('text', 'price', 'Price', 6, [], ['key' => 'price'])) -->
                                                    </div>
                                                </div>
                                            @endforeach
                                    ...
                                    <!-- @include('admin.layout.field', field('select', 'card_id', 'Cards', 12, [], $cards, true)) -->
                                           ...
                                           <label for="card_id">Cards<span style="color: red">*</span></label>
                                            <select class="custom-select form-control city-type" id="card_id" name="card_id">
                                                ...
                                                @if(!empty($cards))
                                                    ...
                                                    <option value="{{$card->id}}" {{ old('card_id') == $card->id ? 'selected' : '' }}>{{$card->name}}</option>
                                                    @endforeach
                                                @endif
                                            </select>
