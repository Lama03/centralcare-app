...
@section('pageTitle'){{ $settings['metatitle_blog_page_seo'][app()->getLocale()] ?? __('messages.Central Care Blog') }}@endsection

@section('metaKeys')
    {!! $settings['blogs_page_seo'][app()->getLocale()] ?? '' !!}
@endsection
    ...
    <div class="top-bar__block language nav-link gap-0">
        <ul class="list-inline">
            <li class="list-inline-item large-mg">
                <a href="{{ url(app()->getLocale() != 'ar' ? 'blogs' : 'blogs?lang=en') }}" class="top-bar__side">
...
<div class="simple-background">
    ...
    @include('web.components.page_header', ['page_name' => __('messages.Central Care Blog')])
                ...
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="section-title flex-column align-items-start mb-0">
                            <h2 class="title" data-aos="fade-up">{{ $settings['metatitle_blog_page_seo'][app()->getLocale()] ?? __('messages.Central Care Blog') }}</h2>
                            <div class="sub-desc">{{ $settings['articles_content'][app()->getLocale()] ?? '' }}</div>
                        </div>
                    </div>
                    <div class="col-12 mt-4">
                        <div class="search-block justify-content-start aos-init aos-animate" data-aos="fade-up">
                            <form class="form-elm" method="GET" action="{{ url('blogs') }}">
...
@section('submit.scripts')
    <script type="text/javascript">
        ...
        function showBlogByCat(slugvalue) {
            ReturnUrl = '{{ url("blogs") }}';
